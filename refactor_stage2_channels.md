# Telegram转发工具重构计划 - 第二阶段（频道工具插件实现）

## 任务2.6：实现频道工具插件(ChannelPlugin)

频道工具插件负责频道解析、验证和信息获取等功能。

```python
# tg_app/plugins/utils/channel_plugin.py
import re
...
class ChannelPlugin(PluginBase):
    ...
    async def _handle_channel_check_access(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # 尝试获取最新消息来验证访问权限
            try:
                messages = await self.client.get_chat_history(entity.id, limit=1)
                has_read_access = len(list(messages)) > 0
    ...
    async def _handle_message_get_from_channel(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        reverse = data.get("reverse", False)  # 是否倒序（从旧到新）
    ...
    async def _parse_channel(self, channel_str: str) -> Optional[Chat]:
        ...
        try:
            ...
            # 3. 检查是否是邀请链接 (t.me/xxx 或 https://t.me/xxx 格式)
            invite_match = re.match(r'(?:https?://)?t\.me/(?:joinchat/)?([a-zA-Z0-9_-]+)', channel_str)
            if invite_match:
                invite_link = invite_match.group(1)
                try:
                    if 'joinchat' in channel_str:
                        # 私有链接
                        full_link = f"https://t.me/joinchat/{invite_link}"
                        return await self.client.get_chat(full_link)
                    else:
                        # 公开链接
                        return await self.client.get_chat(invite_link)
                except Exception:
                    logger.warning(f"无法通过邀请链接获取频道: {channel_str}")
                    return None
            
            # 4. 尝试直接获取（可能是完整的邀请链接等）
            try:
...
1. **频道解析**:
   - 支持多种频道格式的解析，包括ID、用户名、邀请链接等
   - 通过正则表达式识别不同格式，并使用适当的方法获取频道实体
   - 处理各种解析错误，确保稳定性

2. **功能模块**:
   - **频道解析**: 将频道标识符转换为Chat对象
   - **频道信息获取**: 获取频道的详细信息
   - **访问权限检查**: 验证对频道的读写权限
   - **消息获取**: 从频道获取消息历史
